...
try {
    ...
    $pythonExecutableExists = file_exists($pythonExecutable);
    if (!$pythonExecutableExists && !in_array($pythonExecutable, ['python', 'python.exe'])) {
        throw new \Exception("Python virtual environment not found at: $pythonExecutable. Please run the setup commands or ensure system Python is available.");
    }
    ...
    if ($jsonOutput === null) {
        throw new \Exception("Error executing Python command or no output received. Command was: $command");
    }
