    ...
    if ($file["error"] !== UPLOAD_ERR_OK) {
        ...
        try {
            ...
            for ($row = 2; $row <= $sheet->getHighestRow(); $row++) {
                ...
                $staff = (int)$sheet->getCell("C" . $row)->getCalculatedValue();
                ...
                if ($existing) {
                    $stmt = $pdo->prepare("UPDATE repo_molsw SET company_name = ?, staff_count = ?, import_batch_id = ? WHERE id = ?");
                    $stmt->execute([$name, $staff, $batch_id, $existing['id']]);
                    $updated++;
                } else {
                    $stmt = $pdo->prepare("INSERT INTO repo_molsw (tin, company_name, staff_count, import_batch_id) VALUES (?, ?, ?, ?)");
                    $stmt->execute([$tin, $name, $staff, $batch_id]);
                    $imported++;
                }
            }
...
$recent_batches = $pdo->query("SELECT import_batch_id, COUNT(*) as count, MAX(created_at) as last_date FROM repo_molsw WHERE import_batch_id IS NOT NULL GROUP BY import_batch_id ORDER BY last_date DESC LIMIT 10")->fetchAll();
...
<div class="row mb-3">
    <div class="col-12">
        <h2><a href="repo_molsw.php" class="text-dark text-decoration-none"><i class="fas fa-arrow-left me-2"></i></a> Import MOLSW Data</h2>
        <p class="text-muted">Upload an Excel file with 3 columns: TIN, Company Name, and Staff Count.</p>
    </div>
</div>
...
<div class="row">
    <div class="col-md-5">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white fw-bold"><i class="fas fa-upload me-2"></i> Upload MOLSW Excel</div>
            <div class="card-body">
    ...
    <div class="col-md-7">
        <div class="card shadow-sm">
            <div class="card-header bg-white fw-bold"><i class="fas fa-history me-2"></i> Recent MOLSW Imports</div>
