...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_batch'])) {
    $bid = $_POST['batch_id'];
    $pdo->prepare("DELETE FROM import_royalty_data WHERE batch_id = ?")->execute([$bid]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES["excel_file"])) {
    ...
    if ($file["error"] !== UPLOAD_ERR_OK) {
        ...
        try {
            ...
            $batch_id = "ROYAL_BATCH_" . date("YmdHis");
            $imported = 0;
...
$recent = $pdo->query("SELECT batch_id, tax_year, COUNT(*) as `rows`, MAX(import_date) as latest FROM import_royalty_data GROUP BY batch_id, tax_year ORDER BY latest DESC LIMIT 5")->fetchAll();
...
<div class="row mb-3">
  <div class="col-md-8">
    <h2><i class="fas fa-file-excel me-2 text-danger"></i> Import Royalty Fee Data</h2>
    <p class="text-muted">Import natural resource royalty fee data for Tax Expenditure estimation.</p>
  </div>
  <div class="col-md-4 text-end">
    <a href="add_edit_royalty.php" class="btn btn-outline-danger"><i class="fas fa-plus me-1"></i> Add Manual Record</a>
  </div>
</div>
...
<div class="row g-4">
  <div class="col-md-4">
    <div class="card shadow-sm border-0 border-top border-4 border-danger h-100">
      <div class="card-header bg-white text-dark fw-bold">Upload Royalty Fee Excel</div>
      <div class="card-body">
        <form method="POST" enctype="multipart/form-data" id="uploadForm">
          <div class="mb-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <label class="form-label fw-bold mb-0 small">Excel File (.xlsx)</label>
                <a href="<?= BASE_URL ?>/docs/Royalty%20fee_Template.xlsx" class="btn btn-sm btn-link text-danger p-0 text-decoration-none fw-bold small"><i class="fas fa-download me-1"></i> Download Template</a>
            </div>
          ...
          <div class="d-grid">
            <button type="submit" class="btn btn-danger text-white shadow-sm fw-bold" id="importBtn"><i class="fas fa-file-import me-2"></i> Import Batch</button>
          </div>
        </form>
        ...
        <?php else: ?>
            ...
            <div class="d-flex justify-content-between align-items-center mb-2 p-2 rounded <?= $current_batch == $r['batch_id'] ? 'bg-light border border-danger' : '' ?>">
                <div class="small">
    ...
    <div class="card shadow-sm border-0 h-100">
      <div class="card-header bg-white d-flex justify-content-between align-items-center">
        ...
        <?php if ($current_batch): ?>
            <a href="te_royalty.php?batch=<?= urlencode($current_batch) ?>" class="btn btn-sm btn-primary"><i class="fas fa-calculator me-1"></i> Go to Calculation</a>
        <?php endif; ?>
      </div>
                ...
                <tbody class="small">
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td><?= number_format($r['electricity_sale_value'], 0) ?></td>
                            <td class="text-end"><?= number_format($r['actual_rate'], 2) ?>%</td>
                            <td class="text-end"><?= number_format($r['fee_collected'], 2) ?></td>
                            <td class="text-center">
                                <a href="add_edit_royalty.php?id=<?= $r['id'] ?>" class="text-danger"><i class="fas fa-edit"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
