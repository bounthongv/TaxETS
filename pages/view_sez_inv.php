...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
    try {
        ...
        if ($action === "update_record") {
            ...
            $new_batch = $batch ?: "MANUAL_ENTRY_SEZINV_" . date("Y");
            ...
            if (empty($batch)) {
                header("Location: view_sez_inv.php?batch=" . urlencode($new_batch));
                exit;
            }
...
$records = [];
if ($batch) {
    $stmt = $pdo->prepare("SELECT * FROM import_sez_data WHERE batch_id = ? AND type = 'Investor' ORDER BY id");
    $stmt->execute([$batch]);
...
$years = [];
if ($batch) {
    $stmt = $pdo->prepare("SELECT DISTINCT tax_year FROM import_sez_data WHERE batch_id = ? AND type = 'Investor' ORDER BY tax_year DESC");
    $stmt->execute([$batch]);
...
<div class="row mb-3">
  <div class="col-12 d-flex justify-content-between align-items-center">
    <div>
      <h2><a href="import_sez_inv.php" class="text-dark text-decoration-none"><i class="fas fa-arrow-left me-2"></i></a> SEZ Investor Records</h2>
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header bg-light">
        <h5 class="modal-title" id="modalTitle">SEZ Investor Record</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form method="POST">
        ...
        <div class="modal-body">
          ...
          <div class="card bg-light border-0 mb-3">
            <div class="card-header bg-transparent fw-bold py-2">Utility & Infrastructure (LAK)</div>
            <div class="card-body row">
              <div class="col-md-6 mb-2">
                <label class="form-label small text-muted">Electricity and Water used in Production</label>
                <input type="number" step="0.01" name="amount_utility_usage" id="edit_amount_utility_usage" class="form-control">
              </div>
              <div class="col-md-6 mb-2">
                <label class="form-label small text-muted">Construction/Development of Infrastructures</label>
                <input type="number" step="0.01" name="amount_infra_dev" id="edit_amount_infra_dev" class="form-control">
              </div>
            </div>
          </div>
        </div>
...
function editRecord(id) {
    ...
    document.getElementById('edit_amount_utility_usage').value = r.amount_utility_usage || 0;
    document.getElementById('edit_amount_infra_dev').value = r.amount_infra_dev || 0;
...
function goToManualEntry() {
    ...
    window.location.href = `view_sez_inv.php?batch=MANUAL_ENTRY_SEZINV_${year}&auto_add=1&year=${year}`;
}
