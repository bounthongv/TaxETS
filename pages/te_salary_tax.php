...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['clear_results'])) {
    $bid = $_POST['batch_id'];
    $pdo->prepare("UPDATE import_salary_tax_data SET benchmark_tax = 0, te_amount = 0, calculated_at = NULL WHERE batch_id = ?")->execute([$bid]);
...
$recent = $pdo->query("SELECT batch_id, tax_year, COUNT(*) as `rows`, SUM(te_amount) as total_te, MAX(import_date) as latest, MAX(calculated_at) as last_calc FROM import_salary_tax_data GROUP BY batch_id, tax_year ORDER BY latest DESC LIMIT 10")->fetchAll();
...
<div class="row mb-3 align-items-center">
    <div class="col-md-8">
        <h2><i class="fas fa-calculator me-2 text-primary"></i> Salary Tax TE</h2>
    ...
    <div class="card shadow-sm border-0 h-100">
      ...
      <div class="card-body p-2">
        ...
        <?php else: ?>
            ...
            <div class="mb-2 p-2 rounded <?= $current_batch == $r['batch_id'] ? 'bg-light border border-primary' : 'border' ?>">
                <div class="d-flex justify-content-between align-items-center">
                    <a href="?batch=<?= urlencode($r['batch_id']) ?>" class="small fw-bold text-decoration-none"><?= substr($r['batch_id'], -14) ?> (<?= $r['tax_year'] ?>)</a>
                    <?php if ($r['last_calc']): ?>
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-md-6 text-end">
                        <form method="POST" class="d-inline">
                            ...
                            <button type="submit" name="calculate_batch" class="btn <?= $is_calculated ? 'btn-outline-primary' : 'btn-primary' ?> shadow-sm">
                    ...
                    <div class="row g-3 mt-3">
                        ...
                        <div class="col-md-3">
                            <div class="p-3 bg-light rounded border-start border-4 border-info">
                                <div class="small text-muted text-uppercase fw-bold">Total Taxable</div>
                                <div class="h4 mb-0 fw-bold"><?= number_format($summary['total_taxable'], 2) ?></div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="p-3 bg-light rounded border-start border-4 border-warning">
                                <div class="small text-muted text-uppercase fw-bold">Total Exempt</div>
                                <div class="h4 mb-0 fw-bold"><?= number_format($summary['total_exempt'], 2) ?></div>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
                    ...
                    <table class="table table-hover table-striped mb-0 align-middle">
                        ...
                        <tbody class="small">
                            ...
                            <tr>
                                ...
                                <td class="text-end"><?= number_format($r['tax_exempt_amount'], 2) ?></td>
                                <td class="text-end fw-bold"><?= number_format($r['tax_amount'], 2) ?></td>
                                <td class="text-end text-primary fw-bold"><?= number_format($r['benchmark_tax'], 2) ?></td>
                                <td class="text-end text-success fw-bold"><?= number_format($r['te_amount'], 2) ?></td>
    ...
    <?php else: ?>
        <div class="card shadow-sm border-0 border-top border-4 border-primary">
            <div class="card-body p-5 text-center">
                ...
                <p class="text-muted">Please select a salary tax batch from the sidebar on the left.</p>
                <a href="import_salary.php" class="btn btn-primary mt-3"><i class="fas fa-upload me-2"></i> Go to Import Page</a>
            </div>
        </div>
    <?php endif; ?>
  </div>
