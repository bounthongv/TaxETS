...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['delete_batch'])) {
    $bid = $_POST['batch_id'];
    $pdo->prepare("DELETE FROM import_sez_data WHERE batch_id = ? AND type = 'Investor'")->execute([$bid]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_FILES["excel_file"])) {
    ...
    if ($file["error"] !== UPLOAD_ERR_OK) {
        ...
        try {
            ...
            $batch_id = "SEZ_INV_BATCH_" . date("YmdHis");
...
$recent = $pdo->query("SELECT batch_id, COUNT(*) as `rows`, MAX(import_date) as latest FROM import_sez_data WHERE type='Investor' GROUP BY batch_id ORDER BY latest DESC LIMIT 5")->fetchAll();
...
<div class="row mb-3">
  <div class="col-md-8">
    <h2><i class="fas fa-file-excel me-2 text-success"></i> Import Data for SEZ Investors</h2>
    <p class="text-muted">Import utility usage and infrastructure data using the standardized investor template.</p>
  </div>
  <div class="col-md-4 text-end">
    <a href="add_edit_sez_inv.php" class="btn btn-outline-success"><i class="fas fa-plus me-1"></i> Add Manual Record</a>
  </div>
</div>
...
<div class="row g-4">
  <div class="col-md-4">
    <div class="card shadow-sm border-0 border-top border-4 border-success h-100">
      <div class="card-header bg-white text-dark fw-bold">Upload Investor Excel</div>
      <div class="card-body">
        <form method="POST" enctype="multipart/form-data" id="uploadForm">
          <div class="mb-3">
            <div class="d-flex justify-content-between align-items-center mb-2">
                <label class="form-label fw-bold mb-0 small">Excel File (.xlsx)</label>
                <a href="<?= BASE_URL ?>/docs/For%20SEZ%20Investors_Template.xlsx" class="btn btn-sm btn-link text-success p-0 text-decoration-none fw-bold small"><i class="fas fa-download me-1"></i> Download Template</a>
            </div>
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="add_edit_sez_inv.php?id=<?= $r['id'] ?>" class="text-success"><i class="fas fa-edit"></i></a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
