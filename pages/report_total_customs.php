...
$all_years = [];
try {
    $stmt = $pdo->query("SELECT DISTINCT YEAR(i.doc_date) FROM asycuda_imports i JOIN te_asycuda_result te ON i.id = te.asycuda_id WHERE te.total_te <> 0 AND i.doc_date IS NOT NULL");
...
try {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { $regimeDescriptions[$row['regime_code']] = $row['description']; }
    $stmt = $pdo->query("SELECT condition_code, description FROM bm_payment_condition_codes WHERE active = 1");
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { $conditionDescriptions[$row['condition_code']] = $row['description']; }
} catch (Exception $e) { $errors[] = $e->getMessage(); }

// Build matrix: [regime_4digit][regime_3digit][year] => total_te
$matrix = [];
$regimeYearTotals = [];
$grandYearTotals = [];

// Per-tax-type totals for chart breakdown
$typeTotals = ['customs' => [], 'excise' => [], 'vat' => []];
...
try {
    ...
    $sql = "SELECT
                ...
                SUM(te.customs_te) AS customs_te,
                SUM(te.excise_te) AS excise_te,
                SUM(te.vat_te) AS vat_te,
                SUM(te.total_te) AS total_te
            FROM asycuda_imports i
            JOIN te_asycuda_result te ON i.id = te.asycuda_id
            WHERE te.total_te <> 0 AND i.doc_date IS NOT NULL
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $typeTotals['customs'][$year] = ($typeTotals['customs'][$year] ?? 0) + (float)$row['customs_te'];
        $typeTotals['excise'][$year] = ($typeTotals['excise'][$year] ?? 0) + (float)$row['excise_te'];
        $typeTotals['vat'][$year] = ($typeTotals['vat'][$year] ?? 0) + (float)$row['vat_te'];
    }
} catch (Exception $e) { $errors[] = $e->getMessage(); }

ksort($matrix);

// Chart data: total per regime (4-digit) per year
$chartData = [];
...
$compChartData = [
    ['type' => 'customs', 'label' => 'Customs Duty'],
    ['type' => 'excise', 'label' => 'Excise Tax'],
    ['type' => 'vat', 'label' => 'Import VAT'],
];
foreach ($compChartData as &$t) {
    foreach ($display_years as $y) { $t[(string)$y] = (float)($typeTotals[$t['type']][$y] ?? 0); }
}
unset($t);
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
    ...
    $sheet->setTitle("Total TE by Customs Regime");
    $sheet->mergeCells('A1:' . chr(65 + count($display_years)) . '1');
    $sheet->setCellValue('A1', "Total TE by Customs Regime Code ({$from_year} - {$to_year})");
    ...
    foreach ($matrix as $r4 => $subRegimes) {
        ...
        foreach ($subRegimes as $r3 => $yearData) {
            ...
            $cIdx = 2; $subTotal = 0;
            foreach ($display_years as $y) { $val = $yearData[$y] ?? 0; $sheet->setCellValueByColumnAndRow($cIdx, $rowIdx, $val); $cIdx++; $subTotal += $val; }
        ...
        $sheet->getStyle("A{$rowIdx}")->getFont()->setBold(true);
        $cIdx = 2; $r4Total = 0;
        foreach ($display_years as $y) { $val = $regimeYearTotals[$r4][$y] ?? 0; $sheet->setCellValueByColumnAndRow($cIdx, $rowIdx, $val); $cIdx++; $r4Total += $val; }
    ...
    header('Content-Disposition: attachment; filename="Total_Customs_TE_by_Regime_' . $from_year . '-' . $to_year . '.xlsx"');
...
?>
<div class="row mb-4">
    <div class="col-md-8"><h2 class="fw-bold text-dark"><i class="fas fa-layer-group me-2 text-primary"></i> Total TE by Customs Regime Code</h2><p class="text-muted">Aggregated Tax Expenditure from Customs Duty, Excise Tax, and Import VAT, grouped by Regime Code (4-digit) and Sub-Regime / Payment Condition (3-digit).</p></div>
...
<form method="GET" class="row align-items-end g-3">
    ...
    <?= reportImportDateFilterControl("report_total_customs.php", $from_year, $to_year) ?>
    ...
    <div class="col-md-2"><a href="report_total_customs.php" class="btn btn-outline-secondary w-100 border-0">Reset</a></div>
</form></div></div>
...
<div class="d-flex justify-content-between align-items-center mb-3"><h5 class="fw-bold mb-0"><i class="fas fa-chart-bar me-2 text-primary"></i> TE by Regime Code (<?= $from_year ?>&ndash;<?= $to_year ?>)</h5>
...
<div id="chartContainer" style="width:100%;height:360px;"><canvas id="teTotalCustomsChart"></canvas></div>
<div id="pieChartsContainer" class="row g-3" style="display:none;"></div>
</div></div>

<div class="card shadow-sm mb-4" style="border-radius:12px;"><div class="card-body">
<h5 class="fw-bold mb-3"><i class="fas fa-chart-pie me-2 text-primary"></i> Composition by Tax Type (<?= $from_year ?>&ndash;<?= $to_year ?>)</h5>
<div style="width:100%;height:300px;"><canvas id="teCompositionChart"></canvas></div>
</div></div>
...
<script>
var chartData=<?= json_encode($chartData) ?>,compData=<?= json_encode($compChartData) ?>,chartYears=<?= json_encode($display_years) ?>,chart=null,compChart=null,pieCharts=[],currentChartType='bar';
...
var COMP_COLORS=['#4e79a7','#f28e2b','#e15759'];
...
function renderChart(){if(chart){chart.destroy();chart=null}pieCharts.forEach(function(c){c.destroy()});pieCharts=[];var fd=chartData.filter(function(d){return chartYears.some(function(y){return Math.abs(d[y])>0})});if(fd.length===0)fd=chartData;if(currentChartType==='line'){document.getElementById('chartContainer').style.display='block';document.getElementById('pieChartsContainer').style.display='none';document.getElementById('pieChartsContainer').innerHTML='';var ds=fd.map(function(d,i){return{label:d.label,data:chartYears.map(function(y){return d[y]||0}),borderColor:getColor(i),backgroundColor:getColor(i),fill:false,tension:.2,spanGaps:true}});chart=new Chart(document.getElementById('teTotalCustomsChart'),{type:'line',data:{labels:chartYears,datasets:ds},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{display:true,position:'top',labels:{boxWidth:12,padding:10,font:{size:11}}},tooltip:{callbacks:{label:function(ctx){return ctx.dataset.label+': '+Number(ctx.raw).toLocaleString()}}}},scales:{x:{title:{display:true,text:'Year',font:{size:11}},ticks:{font:{size:11}}},y:{beginAtZero:true,ticks:{callback:function(v){return formatValue(v)},font:{size:11}}}}}})}else if(currentChartType==='bar'){document.getElementById('chartContainer').style.display='block';document.getElementById('pieChartsContainer').style.display='none';document.getElementById('pieChartsContainer').innerHTML='';var ds=chartYears.map(function(y,i){return{label:String(y),data:fd.map(function(d){return d[y]||0}),backgroundColor:getColor(i)}});chart=new Chart(document.getElementById('teTotalCustomsChart'),{type:'bar',data:{labels:fd.map(function(d){return d.label}),datasets:ds},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{display:true,position:'top',labels:{boxWidth:12,padding:10,font:{size:11}}},tooltip:{callbacks:{label:function(ctx){return ctx.dataset.label+': '+Number(ctx.raw).toLocaleString()}}}},scales:{x:{ticks:{font:{size:10}}},y:{beginAtZero:true,ticks:{callback:function(v){return formatValue(v)},font:{size:11}}}}}})}else if(currentChartType==='pie'){document.getElementById('chartContainer').style.display='none';document.getElementById('pieChartsContainer').style.display='flex';document.getElementById('pieChartsContainer').innerHTML='';chartYears.forEach(function(year){var yd=fd.map(function(d){return{label:d.label,value:d[year]||0}}).filter(function(i){return i.value>0});if(yd.length===0)return;var col=document.createElement('div');col.className='col-lg-4 col-md-6 col-sm-12';var ctn=document.createElement('div');ctn.className='pie-chart-container';var cv=document.createElement('canvas');ctn.appendChild(cv);col.appendChild(ctn);document.getElementById('pieChartsContainer').appendChild(col);var p=new Chart(cv,{type:'pie',data:{labels:yd.map(function(i){return i.label}),datasets:[{data:yd.map(function(i){return i.value}),backgroundColor:yd.map(function(_,i){return getColor(i)})}]},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{position:'right',labels:{boxWidth:8,padding:4,font:{size:9}}},title:{display:true,text:'Year '+year,font:{size:11},padding:{top:4,bottom:2}},tooltip:{callbacks:{label:function(ctx){var total=ctx.dataset.data.reduce(function(a,b){return a+b},0);return ctx.label+': '+Number(ctx.raw).toLocaleString()+' ('+((ctx.raw/total)*100).toFixed(1)+'%)'}}}}}});pieCharts.push(p)})}}
renderChart();
// Composition chart (stacked bar)
if(compChart){compChart.destroy()}
var compDs=compData.map(function(d,i){return{label:d.label,data:chartYears.map(function(y){return d[y]||0}),backgroundColor:COMP_COLORS[i]}});
compChart=new Chart(document.getElementById('teCompositionChart'),{type:'bar',data:{labels:chartYears,datasets:compDs},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{display:true,position:'top',labels:{boxWidth:12,padding:10,font:{size:11}}},tooltip:{callbacks:{label:function(ctx){return ctx.dataset.label+': '+Number(ctx.raw).toLocaleString()}}}},scales:{x:{stacked:true,title:{display:true,text:'Year',font:{size:11}},ticks:{font:{size:11}}},y:{stacked:true,beginAtZero:true,ticks:{callback:function(v){return formatValue(v)},font:{size:11}}}}}});
