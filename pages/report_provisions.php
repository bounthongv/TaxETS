...
$all_years = [];
try {
    $stmt = $pdo->query("SELECT DISTINCT tax_year FROM companies WHERE tax_year > 0");
...
// 2. Fetch all provisions with short labels
$provisions = [];
try {
    $stmt = $pdo->query("SELECT id, provision_number, legal_reference, description, is_exemption, target_rate 
                         FROM profit_provisions ORDER BY CAST(provision_number AS UNSIGNED) ASC, provision_number ASC");
    $provisions = $stmt->fetchAll();
} catch (Exception $e) { $errors[] = $e->getMessage(); }

// Build label map: provision_number => short label
$provLabel = [];
$provDesc = [];
foreach ($provisions as $p) {
    $num = $p['provision_number'];
    $short = "Prov #{$num}";
    if (!empty($p['legal_reference'])) $short .= " \u{2013} " . $p['legal_reference'];
    $provLabel[$num] = $short;
    $provDesc[$num] = $p['description'];
}

// 3. Aggregate data: matrix[provision_number][year] = TE
$matrix = [];
$other_te = [];     // Unclassified: companies with no matched provision
$grand_total_te = []; // Unique total from all companies
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
    ...
    $sheet->setCellValue('A1', "Profit Tax TE by Provision ({$from_year} - {$to_year})");
    ...
    // Grand total row
    $sheet->setCellValue("A{$rowIdx}", 'TOTAL (all companies)');
    ...
    header('Content-Disposition: attachment; filename="TE_by_Provision_' . $from_year . '-' . $to_year . '.xlsx"');
...
<div class="row mb-4">
    <div class="col-md-8">
        <h2 class="fw-bold text-dark"><i class="fas fa-gavel me-2 text-primary"></i> Profit Tax TE by Provision</h2>
...
<div class="card shadow-sm border-0 mb-4" style="border-radius: 12px;">
    <div class="card-body bg-light rounded-3">
        <form method="GET" class="row align-items-end g-3">
            ...
            <div class="col-md-3">
                <a href="report_provisions.php" class="btn btn-outline-secondary w-100 border-0">Reset</a>
            </div>
        </form>
    </div>
</div>
...
<div id="reportContent" class="card shadow-sm border-0" style="border-radius: 12px;">
    <div class="card-body p-0">
            ...
            <table class="table table-hover align-middle mb-0 matrix-table">
                ...
                <tbody>
                    ...
                    <tr class="table-warning bg-opacity-10 border-top">
                        <td class="ps-4 fw-bold text-muted">
                            ...
                            <span class="text-muted small fw-normal ms-2" title="TE from companies that did not match any provision.">(no provision matched)</span>
                        </td>
                        <?php $unRowTotal = 0; ?>
                        <?php foreach ($display_years as $year):
                            $val = $other_te[$year] ?? 0;
                ...
                <tfoot class="bg-light border-top-2 border-dark">
                    <tr class="fw-bold text-dark align-middle">
                        <td class="ps-4 py-3">GRAND TOTAL <span class="text-muted small fw-normal">(unique, all companies)</span></td>
                        <?php
                        $grandTotal = 0;
        ...
        <div class="px-4 py-2 bg-light border-top small text-muted">
            <i class="fas fa-info-circle me-1"></i> A company may match multiple provisions. The provision rows show the full TE for every company matching that provision, so the sum of provision rows may exceed the Grand Total.
        </div>
    </div>
</div>
                    ...
                    html2canvas(document.getElementById('reportContent'), {
                        ...
                        if (chartCanvas) {
                            setTimeout(function() {
                                ...
                                pdf.save('TE_by_Provision_<?= $from_year ?>-<?= $to_year ?>.pdf');
                                btn.disabled = false;
