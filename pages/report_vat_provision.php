...
$all_years = [];
try {
    $stmt = $pdo->query("SELECT DISTINCT YEAR(filing_period) as yr FROM import_vat_data WHERE filing_period IS NOT NULL AND filing_period != '0000-00-00'");
...
// Fetch VAT provisions
$provisions = [];
try {
    $stmt = $pdo->query("SELECT id, provision_number, legal_basis, description, type_of_te FROM vat_provisions ORDER BY CAST(provision_number AS UNSIGNED) ASC, provision_number ASC");
...
try {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $matrix[$row['provision_number']][(int)$row['yr']] = (float)$row['te'];
    }
    ...
    $stmt->execute($params);
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $other_te[(int)$row['yr']] = (float)$row['te'];
    }
    ...
    $stmt->execute($params);
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $grand_total_te[(int)$row['yr']] = (float)$row['te'];
    }
} catch (Exception $e) { $errors[] = $e->getMessage(); }

$chartData = [];
$allProvNums = array_keys($matrix);
usort($allProvNums, function($a, $b) { $aNum = (int)$a; $bNum = (int)$b; if ($aNum !== $bNum) return $aNum - $bNum; return strcasecmp($a, $b); });
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
    ...
    $sheet->setCellValue('A1', "VAT TE by Provision ({$from_year} - {$to_year})");
    $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);
    $sheet->getStyle('A1')->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
    $headerRow = 3;
    $sheet->setCellValue("A{$headerRow}", 'Provision'); $colIdx = 2;
    ...
    foreach ($matrix as $num => $yearData) {
        $label = $provLabel[$num] ?? "Prov #{$num}"; $sheet->setCellValue("A{$rowIdx}", $label);
        $cIdx = 2; $rowTotal = 0;
    ...
    header('Content-Disposition: attachment; filename="TE_VAT_by_Provision_' . $from_year . '-' . $to_year . '.xlsx"');
...
?>
<div class="row mb-4">
    <div class="col-md-8"><h2 class="fw-bold text-dark"><i class="fas fa-chart-line me-2 text-primary"></i> Domestic VAT TE by Provision</h2><p class="text-muted">Tax Expenditure from Domestic VAT, classified by provision.</p></div>
    <div class="col-md-4 text-end"><a href="?<?= reportAppendFilters(["export" => 1, "from_year" => $from_year, "to_year" => $to_year]) ?>" class="btn btn-success shadow-sm"><i class="fas fa-file-excel me-2"></i> Export Excel</a><button id="exportPdfBtn" class="btn btn-danger shadow-sm ms-2"><i class="fas fa-file-pdf me-2"></i> Export PDF</button></div>
</div>
<?php if (!empty($errors)): ?><div class="alert alert-danger shadow-sm"><?= htmlspecialchars(implode('; ', $errors)) ?></div><?php endif; ?>
<div class="card shadow-sm border-0 mb-4" style="border-radius: 12px;"><div class="card-body bg-light rounded-3"><form method="GET" class="row align-items-end g-3">
    ...
    <?= reportImportDateFilterControl("report_vat_provision.php", $from_year, $to_year) ?>
    ...
    <div class="col-md-3"><a href="report_vat_provision.php" class="btn btn-outline-secondary w-100 border-0">Reset</a></div>
</form></div></div>
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3"><div class="card shadow-sm border-0 h-100" style="border-radius:12px;background:linear-gradient(135deg,#43e97b 0%,#38f9d7 100%);"><div class="card-body text-white d-flex flex-column"><div class="d-flex justify-content-between align-items-center mb-2"><span class="fw-light small text-white-50 text-uppercase">Unclassified</span><i class="fas fa-folder-open fa-2x text-white-50"></i></div><span class="fw-bold fs-4"><?= number_format(array_sum($other_te), 0) ?></span><small class="text-white-50 mt-1">No provision</small></div></div></div>
</div>
<div id="reportContent" class="card shadow-sm border-0" style="border-radius:12px;"><div class="card-body p-0"><div class="table-responsive"><table class="table table-hover align-middle mb-0 matrix-table"><thead class="bg-primary text-white"><tr class="align-middle"><th class="ps-4 py-3" style="min-width:240px;">Provision</th><?php foreach ($display_years as $year): ?><th class="text-end py-3"><?= $year ?></th><?php endforeach; ?><th class="text-end pe-4 py-3 bg-dark">Row Total</th></tr></thead><tbody>
<?php $col_totals = []; foreach ($allProvNums as $num): $yearData = $matrix[$num] ?? []; $rowTotal = 0; ?>
<tr><td class="ps-4 fw-bold text-primary" title="<?= htmlspecialchars($provDesc[$num] ?? '') ?>"><?= htmlspecialchars($provLabel[$num] ?? "Prov #{$num}") ?></td>
...
<?php endforeach; ?><td class="text-end pe-4 fw-bold bg-light"><?= $rowTotal > 0 ? number_format($rowTotal, 0) : '-' ?></td></tr>
<?php endforeach; ?>
<?php if (array_sum($other_te) > 0): ?>
<tr class="table-warning bg-opacity-10 border-top"><td class="ps-4 fw-bold text-muted"><i class="fas fa-question-circle me-1 text-warning"></i> Unclassified / Other</td>
<?php $unRowTotal = 0; foreach ($display_years as $year): $val = $other_te[$year] ?? 0; $unRowTotal += $val; $col_totals[$year] = ($col_totals[$year] ?? 0) + $val; ?>
<td class="text-end <?= $val > 0 ? '' : 'text-muted opacity-25' ?>"><?= $val > 0 ? number_format($val, 0) : '-' ?></td>
<?php endforeach; ?><td class="text-end pe-4 fw-bold"><?= number_format($unRowTotal, 0) ?></td></tr>
<?php endif; ?>
...
<div class="card shadow-sm mb-4 mt-4" style="border-radius:12px;"><div class="card-body"><div class="d-flex justify-content-between align-items-center mb-3"><h5 class="fw-bold mb-0"><i class="fas fa-chart-bar me-2 text-primary"></i> TE by Provision (<?= $from_year ?>&ndash;<?= $to_year ?>)</h5><div class="btn-group btn-group-sm"><button class="btn btn-outline-primary chart-type-btn active" data-type="bar" onclick="switchChartType('bar')"><i class="fas fa-chart-bar"></i> Bar</button><button class="btn btn-outline-primary chart-type-btn" data-type="line" onclick="switchChartType('line')"><i class="fas fa-chart-line"></i> Line</button><button class="btn btn-outline-primary chart-type-btn" data-type="pie" onclick="switchChartType('pie')"><i class="fas fa-chart-pie"></i> Pie</button></div></div><div id="chartContainer" style="width:100%;height:360px;"><canvas id="teProvisionChart"></canvas></div><div id="pieChartsContainer" class="row g-3" style="display:none;"></div></div></div>
<style>.matrix-table{font-size:0.95rem}.matrix-table thead th{border:none;font-weight:700;text-transform:uppercase;letter-spacing:.5px;font-size:.8rem}.matrix-table tbody td{border-bottom:1px solid #f2f2f2}.matrix-table tfoot td{font-size:1.1rem;border-top:2px solid #222}.table-hover tbody tr:hover td{background-color:#f0f7ff}.matrix-table .bg-light{background-color:#f9f9f9!important}</style>
