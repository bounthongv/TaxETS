...
$provision = $pdo->prepare("SELECT * FROM vat_provisions WHERE id = ?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    try {
        if ($_POST["action"] === "add_condition") {
            $stmt = $pdo->prepare("INSERT INTO vat_provision_conditions (provision_id, field_name, operator, value_1, value_2) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([
                $provision_id,
                $_POST["field_name"],
                $_POST["operator"],
            ...
            $stmt = $pdo->prepare("DELETE FROM vat_provision_conditions WHERE id = ? AND provision_id = ?");
...
$conditions = $pdo->prepare("SELECT * FROM vat_provision_conditions WHERE provision_id = ? ORDER BY id ASC");
...
$fieldOptions = [
    ...
    'sales_zero_rate' => 'Sales Zero Rate',
    'purchase_domestic_exempt' => 'Purchase Domestic Exempt',
    'purchase_import_exempt' => 'Purchase Import Exempt',
...
<div class="row mb-3">
    <div class="col-12">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="repo_vat.php">VAT Repository</a></li>
    ...
    <div class="col-12">
        <div class="card shadow-sm border-0" style="border-radius: 12px;">
            <div class="card-header bg-primary text-white" style="border-radius: 12px 12px 0 0;">
                <h5 class="mb-0"><i class="fas fa-file-contract me-2"></i> Provision #<?= htmlspecialchars($provision['provision_number']) ?> - <?= htmlspecialchars($provision['purpose']) ?></h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <table class="table table-sm table-borderless mb-0">
                            ...
                            <tr>
                                ...
                                <td><span class="badge bg-info"><?= htmlspecialchars($provision['type_of_te']) ?></span></td>
                            </tr>
                            <tr>
                    ...
                    <div class="col-md-6">
                        ...
                        <p class="mb-0"><?= htmlspecialchars($provision['description']) ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card shadow-sm border-0 bg-light" style="border-radius: 12px;">
            <div class="card-body">
                ...
                <p class="small text-muted mb-2">
                    When the VAT calculation engine runs, it evaluates each imported record against these conditions.
                    If <strong>ALL conditions match</strong>, the provision is automatically assigned to that record.
                </p>
                <p class="small text-muted mb-0">
                    <strong>Example:</strong> To auto-classify all records with exempt sales greater than 0, add: 
                    <code>sales_exempt > 0</code>
                </p>
            </div>
        </div>
    </div>
