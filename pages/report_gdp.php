...
try {
    ...
    $stmt = $pdo->query("SELECT YEAR(COALESCE(NULLIF(ai.receipt_date, '0000-00-00'), NULLIF(ai.assess_date, '0000-00-00'), ai.doc_date)) as yr, SUM(r.customs_te) as total_te FROM te_asycuda_result r JOIN asycuda_imports ai ON r.asycuda_id = ai.id GROUP BY yr HAVING yr > 0");
    $customs_data = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);

    $stmt = $pdo->query("SELECT YEAR(COALESCE(NULLIF(ai.receipt_date, '0000-00-00'), NULLIF(ai.assess_date, '0000-00-00'), ai.doc_date)) as yr, SUM(r.excise_te) as total_te FROM te_asycuda_result r JOIN asycuda_imports ai ON r.asycuda_id = ai.id GROUP BY yr HAVING yr > 0");
    $excise_data = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);

    $stmt = $pdo->query("SELECT YEAR(COALESCE(NULLIF(ai.receipt_date, '0000-00-00'), NULLIF(ai.assess_date, '0000-00-00'), ai.doc_date)) as yr, SUM(r.vat_te) as total_te FROM te_asycuda_result r JOIN asycuda_imports ai ON r.asycuda_id = ai.id GROUP BY yr HAVING yr > 0");
...
// ===================================================================
// 3. Build chart data: [{name, year: percent}]
// ===================================================================
$chartData = [];
foreach ($tax_types as $label => $data) {
    ...
    foreach ($display_years as $y) {
        $te = (float)($data[$y] ?? 0);
        $gdp = (float)($gdp_data[$y] ?? 0);
        $entry[(string)$y] = $gdp > 0 ? round(($te / $gdp) * 100, 4) : 0;
    }
    $chartData[] = $entry;
}
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
    ...
    foreach ($tax_types as $type => $data) {
        ...
        foreach ($display_years as $year) {
            ...
            $gdp = (float)($gdp_data[$year] ?? 0);
            $pct = $gdp > 0 ? round(($te / $gdp) * 100, 4) : 0;
    ...
    $rowIdx++;
    $sheet->setCellValue('A' . $rowIdx, 'GDP (Billion Kip)');
    $sheet->getStyle('A' . $rowIdx)->getFont()->setBold(true);
    $c = 'B';
    foreach ($display_years as $year) {
    ...
    $rowIdx++;
    $sheet->setCellValue('A' . $rowIdx, 'Total TE (% of GDP)');
    $sheet->getStyle('A' . $rowIdx)->getFont()->setBold(true);
    $c = 'B';
    $lastCol = chr(65 + count($display_years));
    foreach ($display_years as $year) {
        $total_te = 0;
        foreach ($tax_types as $data) { $total_te += (float)($data[$year] ?? 0); }
        $gdp = (float)($gdp_data[$year] ?? 0);
        $pct = $gdp > 0 ? round(($total_te / $gdp) * 100, 4) : 0;
        $sheet->setCellValue($c . $rowIdx, $pct > 0 ? $pct : '');
        $sheet->getStyle($c . $rowIdx)->getNumberFormat()->setFormatCode('0.0000');
        $c++;
    }
    ...
    header('Content-Disposition: attachment; filename="TE_percent_of_GDP.xlsx"');
...
<div class="row mb-3">
  <div class="col-12 d-flex justify-content-between align-items-center">
    <div>
      <h2><i class="fas fa-percentage me-2 text-primary"></i> Tax Expenditure as % of GDP</h2>
      <p class="text-muted">Revenue foregone from each tax type expressed as a percentage of Gross Domestic Product (GDP).</p>
    </div>
      ...
      <table class="table table-bordered table-hover mb-0 align-middle">
        ...
        <tfoot>
          <tr class="table-info fw-bold">
            <td class="ps-4">GDP (Billion Kip)</td>
            <?php foreach ($display_years as $year): ?>
              <td class="text-end pe-4"><?= ($gdp_data[$year] ?? 0) > 0 ? number_format($gdp_data[$year] / 1_000_000_000, 2) : '-' ?></td>
            <?php endforeach; ?>
          </tr>
          <tr class="table-primary fw-bold">
            <td class="ps-4">Total TE (% of GDP)</td>
            <?php foreach ($display_years as $year):
                $total_te = 0;
                foreach ($tax_types as $d) { $total_te += (float)($d[$year] ?? 0); }
                $gdp = (float)($gdp_data[$year] ?? 0);
                $pct = $gdp > 0 ? ($total_te / $gdp) * 100 : 0;
            ?>
...
<div class="card shadow-sm mb-4" style="border-radius: 12px;">
  <div class="card-body">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h5 class="mb-0 fw-bold"><i class="fas fa-chart-line me-2"></i> TE as % of GDP Trend</h5>
    ...
    <div id="chartContainer" style="display: block;">
      <canvas id="teGdpChart" height="120"></canvas>
    </div>
...
(function() {
    ...
    function renderChart() {
        ...
        if (currentChartType === 'line') {
            ...
            chart = new Chart(document.getElementById('teGdpChart'), {
                type: 'line',
...
document.getElementById('exportPdfBtn')?.addEventListener('click', function() {
    ...
    html2canvas(document.getElementById('reportContent'), {
        ...
        var chartCanvas = document.getElementById('teGdpChart');
        if (!chartCanvas) { pdf.save('TE_Percent_of_GDP.pdf'); btn.disabled = false; btn.innerHTML = '<i class="fas fa-file-pdf me-2"></i> Export PDF'; return; }
        ...
        promiseChain.then(function() {
            pdf.save('TE_Percent_of_GDP.pdf');
