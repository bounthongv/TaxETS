...
$all_years = [];
try {
    $stmt = $pdo->query("SELECT DISTINCT tax_year FROM import_salary_tax_data WHERE tax_year > 0");
...
// Get distinct provision numbers
$provision_numbers = [];
try {
    $stmt = $pdo->prepare("SELECT DISTINCT provision_number FROM import_salary_tax_data
                           WHERE provision_number IS NOT NULL AND provision_number != ''
                           ORDER BY provision_number");
    $provision_numbers = $stmt->fetchAll(PDO::FETCH_COLUMN);
} catch (Exception $e) { $errors[] = $e->getMessage(); }
...
// Chart data
$chartData = [];
$allProvNums = $provision_numbers;
foreach ($allProvNums as $num) {
    $item = ['provision_number' => $num, 'label' => "Prov {$num}"];
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
    ...
    $sheet->setCellValue('A1', "Salary Tax TE by Provision ({$from_year} - {$to_year})");
    ...
    foreach ($matrix as $num => $yearData) {
        $sheet->setCellValue("A{$rowIdx}", "Prov {$num}");
        $cIdx = 2; $rowTotal = 0;
    ...
    header('Content-Disposition: attachment; filename="TE_Salary_by_Provision_' . $from_year . '-' . $to_year . '.xlsx"');
...
<div class="row mb-4">
    <div class="col-md-8">
        <h2 class="fw-bold text-dark"><i class="fas fa-money-check-alt me-2 text-primary"></i> Salary Tax TE by Provision</h2>
...
<div class="card shadow-sm border-0 mb-4" style="border-radius: 12px;">
    <div class="card-body bg-light rounded-3">
        <form method="GET" class="row align-items-end g-3">
            ...
            <div class="col-md-3">
                <a href="report_salary_tax_provision.php" class="btn btn-outline-secondary w-100 border-0">Reset</a>
            </div>
        </form>
    </div>
</div>
...
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3">
        <div class="card shadow-sm border-0 h-100" style="border-radius: 12px; background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%);">
            <div class="card-body text-white d-flex flex-column">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <span class="fw-light small text-white-50 text-uppercase">Provisions</span>
                    <i class="fas fa-tag fa-2x text-white-50"></i>
                </div>
                <span class="fw-bold fs-4"><?= count($provision_numbers) ?></span>
                <small class="text-white-50 mt-1">Distinct codes</small>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card shadow-sm border-0 h-100" style="border-radius: 12px; background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%);">
            <div class="card-body text-white d-flex flex-column">
                ...
                <small class="text-white-50 mt-1">With data</small>
            </div>
        </div>
    </div>
    <div class="col-md-3">
        <div class="card shadow-sm border-0 h-100" style="border-radius: 12px; background: linear-gradient(135deg, #43e97b 0%, #38f9d7 100%);">
            <div class="card-body text-white d-flex flex-column">
                ...
                <small class="text-white-50 mt-1">No provision</small>
            </div>
        </div>
    </div>
</div>
            ...
            <table class="table table-hover align-middle mb-0 matrix-table">
                <thead class="bg-primary text-white">
                    <tr class="align-middle">
                        <th class="ps-4 py-3" style="min-width: 200px;">Provision</th>
                ...
                <tbody>
                    ...
                    $col_totals = [];
                    foreach ($allProvNums as $num):
                        $yearData = $matrix[$num] ?? []; $rowTotal = 0;
                    ?>
                    <tr>
                        <td class="ps-4 fw-bold text-primary">Prov <?= htmlspecialchars($num) ?></td>
                        <?php foreach ($display_years as $year):
                            $val = $yearData[$year] ?? 0; $rowTotal += $val; $col_totals[$year] = ($col_totals[$year] ?? 0) + $val;
                        ?>
                    ...
                    <tr class="table-warning bg-opacity-10 border-top">
                        <td class="ps-4 fw-bold text-muted"><i class="fas fa-question-circle me-1 text-warning"></i> Unclassified / Other</td>
                        <?php $unRowTotal = 0; ?>
                        <?php foreach ($display_years as $year):
                            $val = $other_te[$year] ?? 0; $unRowTotal += $val; $col_totals[$year] = ($col_totals[$year] ?? 0) + $val;
                        ?>
                        <td class="text-end <?= $val > 0 ? '' : 'text-muted opacity-25' ?>"><?= $val > 0 ? number_format($val, 0) : '-' ?></td>
                        <?php endforeach; ?>
...
<div class="card shadow-sm mb-4 mt-4" style="border-radius: 12px;">
    <div class="card-body">
        ...
        <div id="chartContainer" style="width:100%; height:360px;"><canvas id="teProvisionChart"></canvas></div>
...
var chart = null, pieCharts = [], currentChartType = 'bar';
...
function renderChart() {
    if(chart){chart.destroy();chart=null;} pieCharts.forEach(function(c){c.destroy();}); pieCharts=[];
    var fd = chartData.filter(function(d){return chartYears.some(function(y){return Math.abs(d[y])>0;});});
    if(fd.length===0) fd=chartData;
    if(currentChartType==='line') {
        document.getElementById('chartContainer').style.display='block'; document.getElementById('pieChartsContainer').style.display='none'; document.getElementById('pieChartsContainer').innerHTML='';
        var ds = fd.map(function(d,i){return{label:d.label,data:chartYears.map(function(y){return d[y]||0;}),borderColor:getColor(i),backgroundColor:getColor(i),fill:false,tension:0.2,spanGaps:true};});
        chart = new Chart(document.getElementById('teProvisionChart'), {type:'line',data:{labels:chartYears,datasets:ds},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{display:true,position:'top',labels:{boxWidth:12,padding:10,font:{size:11}}},tooltip:{callbacks:{label:function(ctx){return ctx.dataset.label+': '+Number(ctx.raw).toLocaleString();}}}},scales:{x:{title:{display:true,text:'Year',font:{size:11}},ticks:{font:{size:11}}},y:{beginAtZero:true,ticks:{callback:function(v){return formatValue(v);},font:{size:11}}}}}});
    } else if(currentChartType==='bar') {
        document.getElementById('chartContainer').style.display='block'; document.getElementById('pieChartsContainer').style.display='none'; document.getElementById('pieChartsContainer').innerHTML='';
        var ds = chartYears.map(function(y,i){return{label:String(y),data:fd.map(function(d){return d[y]||0;}),backgroundColor:getColor(i)};});
        chart = new Chart(document.getElementById('teProvisionChart'), {type:'bar',data:{labels:fd.map(function(d){return d.label;}),datasets:ds},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{display:true,position:'top',labels:{boxWidth:12,padding:10,font:{size:11}}},tooltip:{callbacks:{label:function(ctx){return ctx.dataset.label+': '+Number(ctx.raw).toLocaleString();}}}},scales:{x:{ticks:{font:{size:10}}},y:{beginAtZero:true,ticks:{callback:function(v){return formatValue(v);},font:{size:11}}}}}});
    } else if(currentChartType==='pie') {
        document.getElementById('chartContainer').style.display='none'; document.getElementById('pieChartsContainer').style.display='flex'; document.getElementById('pieChartsContainer').innerHTML='';
        chartYears.forEach(function(year){var yd=fd.map(function(d){return{label:d.label,value:d[year]||0};}).filter(function(i){return i.value>0;});if(yd.length===0)return;var col=document.createElement('div');col.className='col-lg-4 col-md-6 col-sm-12';var ctn=document.createElement('div');ctn.className='pie-chart-container';var cv=document.createElement('canvas');ctn.appendChild(cv);col.appendChild(ctn);document.getElementById('pieChartsContainer').appendChild(col);var p=new Chart(cv,{type:'pie',data:{labels:yd.map(function(i){return i.label;}),datasets:[{data:yd.map(function(i){return i.value;}),backgroundColor:yd.map(function(_,i){return getColor(i);})}]},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{position:'right',labels:{boxWidth:8,padding:4,font:{size:9}}},title:{display:true,text:'Year '+year,font:{size:11},padding:{top:4,bottom:2}},tooltip:{callbacks:{label:function(ctx){var total=ctx.dataset.data.reduce(function(a,b){return a+b;},0);return ctx.label+': '+Number(ctx.raw).toLocaleString()+' ('+((ctx.raw/total)*100).toFixed(1)+'%)';}}}}}});pieCharts.push(p);});
    }
}
