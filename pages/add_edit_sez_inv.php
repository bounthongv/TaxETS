...
if ($id) {
    $stmt = $pdo->prepare("SELECT * FROM import_sez_data WHERE id = ? AND type = 'Investor'");
    $stmt->execute([$id]);
    $record = $stmt->fetch();
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    $data = [
        ...
        "amount_utility_usage" => $_POST["amount_utility_usage"] ?: 0,
        "amount_infra_dev"     => $_POST["amount_infra_dev"] ?: 0,
        "type"                 => 'Investor'
    ];
...
<div class="row mb-3">
  <div class="col-12">
    <h2><i class="fas fa-edit me-2 text-success"></i> <?= $id ? 'Edit' : 'Add' ?> SEZ Investor Record</h2>
    <p class="text-muted">Enter utility and infrastructure development data for an SEZ investor.</p>
  </div>
</div>
...
<div class="card shadow-sm border-0 border-top border-4 border-success">
  <div class="card-body">
    <form method="POST">
      ...
      <div class="row g-3 mb-4">
        <div class="col-md-6">
          <label class="form-label fw-bold text-success">Electricity and Water used in Production</label>
          <input type="number" step="0.01" name="amount_utility_usage" class="form-control border-success" value="<?= $record['amount_utility_usage'] ?? 0 ?>">
        </div>
        <div class="col-md-6">
          <label class="form-label fw-bold text-primary">Construction/Development of Infrastructures</label>
          <input type="number" step="0.01" name="amount_infra_dev" class="form-control border-primary" value="<?= $record['amount_infra_dev'] ?? 0 ?>">
        </div>
      </div>

      <div class="mt-4 pt-3 border-top">
        <button type="submit" class="btn btn-success text-white btn-lg px-5 shadow-sm">
          <i class="fas fa-save me-2"></i> <?= $id ? 'Update Record' : 'Save Record' ?>
        </button>
        <a href="import_sez_inv.php" class="btn btn-outline-secondary btn-lg px-4 ms-2">Cancel</a>
      </div>
    </form>
  </div>
</div>
