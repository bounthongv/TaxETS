...
if ($_SERVER["REQUEST_METHOD"] === "POST" && !empty($_POST["batch_id"])) {
    ...
    try {
        ...
        $calc_result = $engine->calculateBatch($batch_id, 'Developer');
...
// Available batches
$batches = $pdo->query("SELECT batch_id, MIN(tax_year) as min_year, MAX(tax_year) as max_year, COUNT(*) as row_count, MAX(calculated_at) as last_calc FROM import_sez_data WHERE type='Developer' GROUP BY batch_id ORDER BY MAX(id) DESC LIMIT 20")->fetchAll();
...
<div class="row mb-3">
    <div class="col-12">
        ...
        <p class="text-muted">Estimate VAT expenditures for SEZ infrastructure development projects.</p>
    </div>
</div>
...
<?php
    ...
    $total_basic = array_sum(array_column($records, "amount_infra_basic"));
    $total_other = array_sum(array_column($records, "amount_infra_other"));
?>
<div class="row g-3 mb-4">
    ...
    <div class="col-md-3">
        <div class="card border-0 shadow-sm bg-warning text-white h-100" style="border-radius:12px">
            <div class="card-body">
                <div class="small text-white-50 text-uppercase fw-bold">Basic / Other</div>
                <div class="h4 mb-0 fw-bold"><?= number_format($total_basic, 0) ?></div>
                <div class="small text-white-50">Other: <?= number_format($total_other, 0) ?></div>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="card-body">
        <form method="POST" id="calcForm">
            <div class="row align-items-end">
                <div class="col-md-6">
                    <label class="form-label fw-bold">Select SEZ Developer Batch</label>
                ...
                <div class="col-md-3">
                    <div class="alert alert-info mb-0 py-2 small">
                        ...
                        TE = Infrastructure investment × applicable rate.
                    </div>
                </div>
            </div>
        </form>
        ...
        <table class="table table-hover table-striped mb-0 align-middle">
            <thead class="table-dark small">
                <tr>
                    <th>TIN</th>
                    <th>Location</th>
                    <th class="text-end">Infra (Basic)</th>
                    <th class="text-end">Infra (Other)</th>
            ...
            <tbody class="small">
                ...
                <tr>
                    ...
                    <td class="text-end"><?= number_format($r['amount_infra_basic'], 0) ?></td>
                    <td class="text-end"><?= number_format($r['amount_infra_other'], 0) ?></td>
                    <td class="text-end text-primary fw-bold"><?= number_format($r['benchmark_tax'], 0) ?></td>
                    <td class="text-end text-info fw-bold"><?= number_format($r['te_amount'], 0) ?></td>
            ...
            <tfoot class="table-light fw-bold">
                <tr>
                    ...
                    <td class="text-end"><?= number_format(array_sum(array_column($records, "amount_infra_basic")), 0) ?></td>
                    <td class="text-end"><?= number_format(array_sum(array_column($records, "amount_infra_other")), 0) ?></td>
                    ...
                    <td class="text-end text-info"><?= $is_calculated ? number_format(array_sum(array_column($records, "te_amount")), 0) : '<span class="text-muted">—</span>' ?></td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
...
<div class="text-center py-5 text-muted">
    ...
    <h5>Select an SEZ Developer Batch to start calculation.</h5>
</div>
<?php endif; ?>
