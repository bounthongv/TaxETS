...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['clear_results'])) {
    $bid = $_POST['batch_id'];
    $pdo->prepare("UPDATE import_sez_data SET benchmark_tax = 0, te_amount = 0, provision_number = NULL, calculated_at = NULL WHERE batch_id = ? AND type = 'Developer'")->execute([$bid]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['calculate_batch'])) {
    $bid = $_POST['batch_id'];
    $res = $engine->calculateBatch($bid, 'Developer');
...
// Fetch recent batches for the sidebar - use MAX(calculated_at) for status
$recent = $pdo->query("SELECT batch_id, tax_year, COUNT(*) as `rows`, SUM(te_amount) as total_te, MAX(import_date) as latest, MAX(calculated_at) as last_calc FROM import_sez_data WHERE type='Developer' GROUP BY batch_id, tax_year ORDER BY latest DESC LIMIT 10")->fetchAll();
...
<div class="row mb-3 align-items-center">
    <div class="col-md-7">
        <h2><i class="fas fa-calculator me-2 text-info"></i> SEZ Developer TE</h2>
    ...
    <div class="card shadow-sm border-0 h-100">
      ...
      <div class="card-body p-2">
        ...
        <?php else: ?>
            ...
            <div class="mb-2 p-2 rounded <?= $current_batch == $r['batch_id'] ? 'bg-light border border-info' : 'border' ?>">
                    ...
                    <div class="row g-3 mt-3">
                        ...
                        <div class="col-md-4">
                            <div class="p-3 bg-light rounded border-start border-4 border-info">
                                <div class="small text-muted text-uppercase fw-bold">Total Investment</div>
                                <div class="h4 mb-0 fw-bold"><?= number_format($summary['total_investment'], 2) ?></div>
                            </div>
                        </div>
                    </div>
                <?php endif; ?>
    ...
    <?php else: ?>
        <div class="card shadow-sm border-0 border-top border-4 border-info">
            <div class="card-body p-5 text-center">
                ...
                <p class="text-muted">Please select a batch from the sidebar on the left.</p>
                <a href="import_sez_dev.php" class="btn btn-info text-white mt-3"><i class="fas fa-upload me-2"></i> Go to Import Page</a>
            </div>
        </div>
    <?php endif; ?>
  </div>
