...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"]) && $_POST["action"] === "calculate" && !empty($_POST["batch_id"])) {
    try {
        ...
        $summary = $engine->calculateBatch($_POST["batch_id"], 'Developer');
...
// --- Batch List Mode ---
if (!$batch):
    $all_rows = $pdo->query("SELECT * FROM import_sez_data WHERE type = 'Developer' ORDER BY id DESC")->fetchAll();

    $total_records = count($all_rows);
    $total_te = array_sum(array_map(function($r) { return (float)($r['te_amount'] ?? 0); }, $all_rows));
    $total_basic = array_sum(array_map(function($r) { return (float)($r['amount_infra_basic'] ?? 0); }, $all_rows));
    $total_other = array_sum(array_map(function($r) { return (float)($r['amount_infra_other'] ?? 0); }, $all_rows));

    $batches = $pdo->query("SELECT batch_id, MIN(tax_year) as min_year, MAX(tax_year) as max_year,
                            COUNT(*) as `rows`,
                            COALESCE(SUM(te_amount), 0) as total_te,
                            COALESCE(SUM(amount_infra_basic), 0) as total_basic,
                            COALESCE(SUM(amount_infra_other), 0) as total_other
                            FROM import_sez_data
...
<div class="row mb-3">
    <div class="col-md-8">
        <h2><i class="fas fa-calculator me-2 text-info"></i> SEZ Developer TE Calculation</h2>
        <p class="text-muted">Calculate and review Tax Expenditure for SEZ Developer batches.</p>
    </div>
...
<div class="row mb-3 g-2">
    ...
    <div class="col-md-3 col-6">
        <div class="card border-0 shadow-sm bg-info text-white text-center py-3">
            <div class="fs-5 fw-bold"><?= number_format($total_basic, 0) ?></div>
            <div class="small opacity-75">Total Basic Infra</div>
        </div>
    </div>
    <div class="col-md-3 col-6">
        <div class="card border-0 shadow-sm bg-success text-white text-center py-3">
            <div class="fs-5 fw-bold"><?= number_format($total_other, 0) ?></div>
            <div class="small opacity-75">Total Other Infra</div>
        </div>
    </div>
    <div class="col-md-3 col-6">
...
<div class="card shadow-sm">
    <div class="card-header bg-white fw-bold"><i class="fas fa-layer-group me-2 text-secondary"></i> SEZ Developer Batches</div>
    <div class="card-body p-0">
        <?php if (empty($batches)): ?>
        <div class="p-5 text-center text-muted">
            <i class="fas fa-folder-open fa-3x mb-3 opacity-50"></i>
            <h5>No SEZ Developer Data Found</h5>
            <p>Import data from the <a href="import_sez_dev.php">Import &gt; SEZ Developer</a> page first.</p>
        </div>
        <?php else: ?>
        <table class="table table-hover mb-0">
            <thead class="table-light"><tr><th>Batch</th><th>Years</th><th>Records</th><th class="text-end">Total Investment</th><th class="text-end">TE Total</th><th>Actions</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-end fw-bold text-info"><?= number_format($b["total_basic"] + $b["total_other"], 0) ?></td>
...
// --- Batch Detail Mode ---
    ...
    $rows = $pdo->prepare("SELECT * FROM import_sez_data WHERE batch_id = ? AND type = 'Developer' ORDER BY id ASC");
    $rows->execute([$batch]);
    $rows = $rows->fetchAll();

    $total_records = count($rows);
    $total_basic = array_sum(array_map(function($r) { return (float)($r['amount_infra_basic'] ?? 0); }, $rows));
    $total_other = array_sum(array_map(function($r) { return (float)($r['amount_infra_other'] ?? 0); }, $rows));
    $total_te = array_sum(array_map(function($r) { return (float)($r['te_amount'] ?? 0); }, $rows));
    $total_benchmark = array_sum(array_map(function($r) { return (float)($r['benchmark_tax'] ?? 0); }, $rows));
    $total_investment = $total_basic + $total_other;
    ...
    $years = $pdo->prepare("SELECT DISTINCT tax_year FROM import_sez_data WHERE batch_id = ? AND type = 'Developer' ORDER BY tax_year DESC");
    $years->execute([$batch]);
...
<div class="row mb-3">
    <div class="col-md-8">
        <h2><a href="te_sez_dev.php" class="text-dark text-decoration-none"><i class="fas fa-arrow-left me-2"></i></a> SEZ Developer TE: Batch Results</h2>
...
<div class="row mb-3 g-2">
    ...
    <div class="col-md-3 col-6">
        <div class="card border-0 shadow-sm bg-info text-white text-center py-3">
            <div class="fs-5 fw-bold"><?= number_format($total_investment, 0) ?></div>
            <div class="small opacity-75">Total Investment</div>
        </div>
    </div>
    <div class="col-md-3 col-6">
        <div class="card border-0 shadow-sm bg-success text-white text-center py-3">
            <div class="fs-5 fw-bold"><?= number_format($total_benchmark, 0) ?></div>
        ...
        <table id="teTable" class="table table-bordered table-hover w-100" style="font-size:0.85em">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-end"><?= number_format((float)($r["amount_infra_basic"] ?? 0), 0) ?></td>
                    <td class="text-end"><?= number_format((float)($r["amount_infra_other"] ?? 0), 0) ?></td>
                    <td class="text-end fw-bold text-info"><?= number_format((float)($r["benchmark_tax"] ?? 0), 0) ?></td>
                    <td class="text-end fw-bold text-danger"><?= number_format((float)($r["te_amount"] ?? 0), 0) ?></td>
                    <td>
                        <?php if (!empty($r["provision_number"]) && $r["provision_number"] !== 'None'): ?>
            ...
            <tfoot>
                <tr class="table-warning fw-bold">
                    ...
                    <td class="text-end text-info"><?= number_format($total_basic, 0) ?></td>
                    <td class="text-end text-success"><?= number_format($total_other, 0) ?></td>
                    <td class="text-end text-info"><?= number_format($total_benchmark, 0) ?></td>
