...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
    try {
        if ($_POST["action"] === "update_record") {
            ...
            $customs_te = max(0, $exemp_customs - $paid_customs);

            $stmt = $pdo->prepare("UPDATE asycuda_imports SET exemp_customs = ?, paid_customs = ? WHERE id = ?");
            $stmt->execute([$exemp_customs, $paid_customs, $id]);

            $stmt_te = $pdo->prepare("UPDATE te_asycuda_result SET customs_te = ?, total_te = customs_te + excise_te + vat_te WHERE asycuda_id = ?");
            $stmt_te->execute([$customs_te, $id]);

            $message = "Customs duty record updated successfully.";
        }
    } catch (Exception $e) {
...
if (!$batch):
    ...
    $total_bm = array_sum(array_column($all_rows, 'exemp_customs'));
    $total_paid = array_sum(array_column($all_rows, 'paid_customs'));

    $batches = $pdo->query("SELECT import_batch_id, MIN(DATE(import_date)) as idate, COUNT(*) as `rows`,
                            SUM(exemp_customs) as total_bm, SUM(paid_customs) as total_paid
                            FROM asycuda_imports i
...
<div class="row mb-3">
    <div class="col-md-8">
        <h2><i class="fas fa-gavel me-2 text-primary"></i> Customs Duty (ASYCUDA)</h2>
        <p class="text-muted">View, edit, and calculate Tax Expenditure for Customs Duty from ASYCUDA imports.</p>
    </div>
...
<div class="row mb-3 g-2">
    ...
    <div class="col-md-4 col-6">
        <div class="card border-0 shadow-sm bg-info text-white text-center py-3">
            ...
            <div class="small opacity-75">Total BM Customs</div>
        </div>
    </div>
    <div class="col-md-4 col-6">
        <div class="card border-0 shadow-sm bg-success text-white text-center py-3">
            ...
            <div class="small opacity-75">Total Paid Customs</div>
        </div>
    </div>
</div>
...
<div class="card shadow-sm">
    ...
    <div class="card-body p-0">
        ...
        <table class="table table-hover mb-0">
            <thead class="table-light"><tr><th>Batch</th><th>Date</th><th>Records</th><th class="text-end">BM Customs</th><th class="text-end">Paid Customs</th><th>Actions</th></tr></thead>
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <a href="te_asycuda_customs.php?batch=<?= urlencode($b["import_batch_id"]) ?>" class="btn btn-sm btn-outline-success" title="Calculate"><i class="fas fa-calculator"></i></a>
                        <?php if ($has_log): ?>
...
// --- Batch Detail Mode ---
    ...
    $total_bm = array_sum(array_column($rows, 'exemp_customs'));
    $total_paid = array_sum(array_column($rows, 'paid_customs'));
    $total_te = array_sum(array_column($rows, 'customs_te'));

    // Distinct regimes for filter
...
<div class="row mb-3">
    <div class="col-md-8">
        <h2><a href="asycuda_customs.php" class="text-dark text-decoration-none"><i class="fas fa-arrow-left me-2"></i></a> Customs Duty: Batch Details</h2>
        <p class="text-muted">Batch: <code><?= htmlspecialchars($batch) ?></code> — <strong><?= $total_records ?></strong> records</p>
    </div>
    <div class="col-md-4 text-end">
        <a href="te_asycuda_customs.php?batch=<?= urlencode($batch) ?>" class="btn btn-success"><i class="fas fa-calculator me-2"></i> Run TE Calculation</a>
    </div>
</div>
...
<div class="row mb-3 g-2">
    ...
    <div class="col-md-4 col-6">
        <div class="card border-0 shadow-sm bg-info text-white text-center py-3">
            <div class="fs-5 fw-bold"><?= number_format($total_bm) ?></div>
            <div class="small opacity-75">BM Customs</div>
        </div>
    </div>
    <div class="col-md-4 col-6">
        <div class="card border-0 shadow-sm bg-success text-white text-center py-3">
            <div class="fs-5 fw-bold"><?= number_format($total_paid) ?></div>
            <div class="small opacity-75">Paid Customs</div>
        </div>
    </div>
</div>
...
<div class="card border-0 shadow-sm">
    <div class="card-body p-0">
        <div class="table-responsive">
            <table id="customsTable" class="table table-bordered table-hover w-100" style="font-size:0.85em">
                <thead class="text-uppercase small">
                    <tr class="text-nowrap">
                        ...
                        <th class="text-end fw-bold">BM Customs</th>
                        <th class="text-end fw-bold">Paid Customs</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end text-success fw-bold"><?= number_format($r['paid_customs']) ?></td>
    ...
    <form method="POST">
      <div class="modal-content">
        <div class="modal-header bg-light">
          <h5 class="modal-title"><i class="fas fa-edit me-2 text-primary"></i> Edit Customs Duty Record</h5>
        ...
        <div class="modal-body">
          ...
          <!-- Customs Duty Fields (Editable) -->
          <div class="card border-0 shadow-sm mb-3">
            <div class="card-header bg-white fw-bold py-2 small">
              <i class="fas fa-edit me-1 text-primary"></i> Customs Duty Fields <small class="text-muted">(editable)</small>
            </div>
            <div class="card-body">
              <div class="row g-2">
                <div class="col-md-4 mb-2">
                  <label class="form-label small fw-bold text-primary">Benchmark Customs</label>
                  <input type="number" step="0.01" name="exemp_customs" id="edit_exemp_customs" class="form-control form-control-sm border-primary">
                </div>
                <div class="col-md-4 mb-2">
                  <label class="form-label small fw-bold text-success">Paid Customs</label>
                  <input type="number" step="0.01" name="paid_customs" id="edit_paid_customs" class="form-control form-control-sm border-success">
                </div>
              </div>
            </div>
          </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const table = $('#customsTable').DataTable({
        dom: 'rtip',
        pageLength: 50,
        order: [],
...
function resetFilters() {
    ...
    $('#customsTable').DataTable().search('').columns().search('').draw();
}
...
.readonly-bg { background-color: #f8f9fa !important; }
</style>
