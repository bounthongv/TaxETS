...
if (isset($_GET["delete"])) {
    try {
        $pdo->prepare("DELETE FROM vat_provisions WHERE id = ?")->execute([$_GET["delete"]]);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["action"])) {
    try {
        if ($_POST["action"] == "add_provision") {
            ...
            $stmt = $pdo->prepare("UPDATE vat_provisions SET provision_number=?, start_year=?, end_year=?, legal_basis=?, description=?, purpose=?, type_of_te=? WHERE id=?");
            $stmt->execute([
                $_POST["provision_number"], $_POST["start_year"], $_POST["end_year"],
                $_POST["legal_basis"], $_POST["description"], $_POST["purpose"], $_POST["type_of_te"], $_POST["id"]
            ]);
            $message = "VAT Provision updated.";
          ...
          <tr class="align-middle">
            ...
            <td class="pe-4 align-middle">
              <div class="d-flex justify-content-end align-items-center">
                  <button class="btn btn-outline-secondary btn-sm me-1" data-bs-toggle="modal" data-bs-target="#editVatProvModal<?= $p['id'] ?>" title="Edit"><i class="fas fa-edit"></i></button>
                  <a href="vat_config_rules.php?provision_id=<?= $p['id'] ?>" class="btn btn-outline-info btn-sm me-1" title="Configure Rules"><i class="fas fa-cogs"></i></a>
                  <a href="?delete=<?= $p['id'] ?>" class="btn btn-outline-danger btn-sm" title="Delete" onclick="return confirm('Delete this VAT provision?')"><i class="fas fa-trash"></i></a>
              </div>
            </td>
          </tr>
          <?php endforeach; ?>
    ...
    <form method="POST">
      ...
      <div class="modal-body p-4">
        <div class="row g-3 mb-3">
            <div class="col-md-4"><label class="form-label small fw-bold text-uppercase">Provision #</label><input type="text" name="provision_number" class="form-control" required placeholder="e.g. 31"></div>
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-6"><label class="form-label small fw-bold text-uppercase">Legal Basis</label><input type="text" name="legal_basis" class="form-control" required placeholder="e.g. VAT Law Art 12.1.1"></div>
            <div class="col-md-6"><label class="form-label small fw-bold text-uppercase">Type of TE</label><select name="type_of_te" class="form-select"><option value="Exemption">Exemption</option><option value="Rate Relief">Rate Relief</option></select></div>
        </div>
        <div class="mb-3"><label class="form-label small fw-bold text-uppercase">Purpose/Objective</label><input type="text" name="purpose" class="form-control" required placeholder="e.g. Food security"></div>
...
<?php foreach ($provisions as $p): ?>
<div class="modal fade" id="editVatProvModal<?= $p['id'] ?>" tabindex="-1">
  <div class="modal-dialog modal-lg"><div class="modal-content border-0 shadow-lg">
    <form method="POST">
      <input type="hidden" name="action" value="edit_provision">
      <input type="hidden" name="id" value="<?= $p['id'] ?>">
      <div class="modal-header bg-warning"><h5 class="modal-title">Edit VAT Provision</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body p-4">
        <div class="row g-3 mb-3">
        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-md-6"><label class="form-label small fw-bold text-uppercase">Type of TE</label><select name="type_of_te" class="form-select"><option value="Exemption" <?= $p['type_of_te']=="Exemption"?"selected":"" ?>>Exemption</option><option value="Rate Relief" <?= $p['type_of_te']=="Rate Relief"?"selected":"" ?>>Rate Relief</option></select></div>
        </div>
        <div class="mb-3"><label class="form-label small fw-bold text-uppercase">Purpose/Objective</label><input type="text" name="purpose" class="form-control" value="<?= htmlspecialchars($p['purpose']) ?>" required></div>
