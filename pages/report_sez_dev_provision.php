...
try {
    ...
    $stmt = $pdo->prepare("SELECT provision_number, tax_year, SUM(te_amount) as te FROM import_sez_data WHERE tax_year BETWEEN ? AND ? AND te_amount > 0 AND type = 'Developer' AND provision_number IS NOT NULL AND provision_number != ''" . reportImportDateCondition(reportBatchDateExpression("import_sez_data", "batch_id", "import_date"), $report_filters, $params) . " GROUP BY provision_number, tax_year");
    $stmt->execute($params); while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { $matrix[$row['provision_number']][(int)$row['tax_year']] = (float)$row['te']; }
    $params = [$from_year, $to_year];
    $stmt = $pdo->prepare("SELECT tax_year, SUM(te_amount) as te FROM import_sez_data WHERE tax_year BETWEEN ? AND ? AND te_amount > 0 AND type = 'Developer' AND (provision_number IS NULL OR provision_number = '')" . reportImportDateCondition(reportBatchDateExpression("import_sez_data", "batch_id", "import_date"), $report_filters, $params) . " GROUP BY tax_year");
    $stmt->execute($params); while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) { $other_te[(int)$row['tax_year']] = (float)$row['te']; }
    $params = [$from_year, $to_year];
    $stmt = $pdo->prepare("SELECT tax_year, SUM(te_amount) as te FROM import_sez_data WHERE tax_year BETWEEN ? AND ? AND te_amount > 0 AND type = 'Developer'" . reportImportDateCondition(reportBatchDateExpression("import_sez_data", "batch_id", "import_date"), $report_filters, $params) . " GROUP BY tax_year");
...
<form method="GET" class="row align-items-end g-3">
    ...
    <?= reportImportDateFilterControl("report_sez_dev_provision.php", $from_year, $to_year) ?>
